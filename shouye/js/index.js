...
var Main = {
	...
	start : function(){//开始
		document.getElementById("js_main").focus();
		 $("#music").attr("src","img/4.mp3");
	...
	over : function(){
		...
		if(this.level >= 3){
			...
			WF.getId("js_end_flush").getElementsByTagName("p")[0].innerHTML = "你牛B呀,到了<label>"+this.level+"</label>级,你请我吃个饭庆祝一下吧？";
			WF.getId("js_end_flush").getElementsByTagName("a")[0].innerHTML = "我要飞上天，和太阳肩并肩";
			WF.getId("js_end_flush").getElementsByTagName("span")[0].className = "icon happy";
		}
		else{
			WF.getId("js_end_flush").getElementsByTagName("p")[0].innerHTML = "你太菜了,才玩了<label>"+this.level+"</label>级,赶快联系我获取秘籍吧。";
			WF.getId("js_end_flush").getElementsByTagName("a")[0].innerHTML = "Try again";
